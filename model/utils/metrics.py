...
def evaluate_pts(gt_bars, gt_ticks, gt_errorup, gt_errordown, pred_bars, pred_ticks, pred_errorup, pred_errordown, dist_thresh = 1.5 / 56):
    ...
    for i in range(len(gt_errorup)):
        min_dist = min([bm for bm in eu_matches[i] if bm > 0], default = 0.)
    ...
    # errordown
    ed_matches = [[((gb[0] - pb[1]) ** 2 + (gb[1] - pb[0]) ** 2) ** (0.5) if
                   ((gb[0] - pb[1]) ** 2 + (gb[1] - pb[0]) ** 2) ** (0.5) < dist_thresh
                   else 0
                   for pb in pred_errordown] for gb in gt_errordown]
    for i in range(len(gt_errordown)):
        min_dist = min([bm for bm in ed_matches[i] if bm > 0], default = 0.)
    ...
    #num_matches = np.count_nonzero(tick_matches) ## ERROR: numpy must be in cpu
