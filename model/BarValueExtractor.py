...
# Model for orientation and origin
class OOExtractor(nn.Module):
    def __init__(self):
        super(OOExtractor, self).__init__()        
    ...
    def forward(self, x):
        ...
        feat = f1 + self.upsample(self.downchannel(f2))
        ...
        origin_pred = torch.sigmoid(origin)
        orient = self.orient(flat56)
        orient_pred = torch.sigmoid(orient)

        return orient_pred, origin_pred
...
class BarValueExtractor(nn.Module):
    def __init__(self):
        ...
        self.downchannel = nn.Conv2d(2048, 256, 1, 1, 0)
        self.upsample = nn.Upsample(size=(56,56), mode='bilinear', align_corners=True)
        #self.reg_upsample = nn.Upsample(size = (2, 56, 56), mode = "bilinear", align_corners = True)
...
#            param.requires_grad = False
        ...
        # output 56x56 pixel map of one point class ['None','t','b','eu','ed'], and regression values (dx, dy)
        #self.pnt_class = nn.Conv2d(256, 3, 1, 1, 0)
        self.pnt_class = nn.Conv2d(256, 1, 1, 1, 0)        
    ...
    def forward(self, x):
        ...
        return pts_cls_pred, pts_reg_pred